...
class RandomJPEGCompression:
    ...
    # Modes PIL can write to JPEG directly; anything else is converted to RGB
    # for the recompression round-trip and converted back afterwards.
    _JPEG_MODES = ("L", "RGB", "CMYK")
    ...
    def __init__(self, min_quality=25, max_quality=95, subsampling=2, seed=None, p = 1.0):
        ...
        if not (1 <= min_quality <= max_quality <= 95):
            raise ValueError(
                f"Quality range must satisfy 1 <= min_quality <= max_quality <= 95, "
                f"got min_quality={min_quality}, max_quality={max_quality}"
            )
        if not (0.0 <= p <= 1.0):
    ...
    def __call__(self, img: Image.Image) -> Image.Image:
        ...
        subsampling = self.subsampling if isinstance(self.subsampling, int) else self.random.choice(self.subsampling)

        # JPEG cannot encode e.g. RGBA/P/LA; round-trip through RGB instead.
        # Note: alpha (if any) is lost and restored as fully opaque.
        src = img if img.mode in self._JPEG_MODES else img.convert("RGB")
