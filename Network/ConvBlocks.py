...
class ConvolutionalBlock(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, stride=1, padding=0, dilation=1, bias=False,
                 padding_mode="replicate", activation=None, norm=None, groups=1):
...
class DepthwiseSeparableConvolution(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size=3, stride=1, padding=1, dilation=1, bias=False,
                 padding_mode="replicate", activation=None, norm=None, num_depthwise=1):
...
class Bottleneck(nn.Module):
    def __init__(self, input_channels, bottleneck_channels, output_channels, repeats=1, bias=False, activation=None,
        ...
        processing_layers = [
            ConvolutionalBlock(bottleneck_channels, bottleneck_channels, kernel_size=3, padding=1, bias=bias,
                               activation=activation, norm=norm) for _ in range(repeats)]
        self.processing_layers = nn.Sequential(*processing_layers)
        self.upscaling = PointwiseConvolutionalBlock(bottleneck_channels, output_channels, bias=bias,
                                                     activation=nn.Identity(),
                                                     norm=norm) if bottleneck_channels != output_channels else nn.Identity()
...
class CSP1_X(nn.Module):
    def __init__(self, input_channels, working_channels, output_channels, X=1, bias=False, activation=None, norm=None):
        ...
        self.rescaling_in = PointwiseConvolutionalBlock(input_channels, working_channels, bias=bias,
                                                        norm=norm) if input_channels != working_channels else nn.Identity()
        branch_A = [ConvolutionalBlock(working_channels, working_channels, kernel_size=3, padding=1, bias=bias,
                                       activation=activation, norm=norm)]
        branch_A.extend([Bottleneck(working_channels, working_channels, working_channels, repeats=2, bias=bias,
                                    activation=activation, shortcut=True, norm=norm) for _ in
                         range(X)])  # Residual blocks, effectively
        ...
        self.rescaling_out = PointwiseConvolutionalBlock(2 * working_channels, output_channels, bias=bias,
                                                         activation=activation, norm=norm)
...
class CSP2_X(nn.Module):
    def __init__(self, input_channels, working_channels, output_channels, X=1, bias=False, activation=None, norm=None):
        ...
        branch_A = [ConvolutionalBlock(working_channels, working_channels, kernel_size=3, padding=1, bias=bias,
                                       activation=activation, norm=norm)]
        branch_A.extend([ConvolutionalBlock(working_channels, working_channels, kernel_size=3, padding=1, bias=bias,
                                            activation=activation, norm=norm) for _ in
                         range(2 * X)])  # Residual blocks, effectively
        self.branch_A = nn.Sequential(*branch_A)
        branch_B = [nn.Conv2d(working_channels, working_channels, kernel_size=1, padding=0, bias=bias)]
        self.branch_B = nn.Sequential(*branch_B)
        self.bn = nn.BatchNorm2d(2 * working_channels, eps=0.001, momentum=0.03, affine=True)
